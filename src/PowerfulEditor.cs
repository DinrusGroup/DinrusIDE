...
namespace DinrusIDE
{
    public partial class PowerfulEditor : Form
    {
        string[] keywords = { "synchronized","abstract", "as", "module", "import", "base", "bool", "break", "byte", "case", "catch", "char", "checked", "class", "const", "continue", "decimal", "default", "delegate", "do", "double", "else", "enum", "event", "explicit", "extern", "false", "finally", "fixed", "float", "for", "foreach", "goto", "if", "implicit", "in", "int", "interface", "internal", "is", "lock", "long", "namespace", "new", "null", "object", "operator", "out", "override", "params", "private", "protected", "public", "readonly", "ref", "return", "sbyte", "sealed", "short", "sizeof", "stackalloc", "static", "string", "struct", "switch", "this", "throw", "true", "try", "typeof", "uint", "ulong", "unchecked", "unsafe", "ushort", "using", "virtual", "void", "volatile", "while", "add", "alias", "ascending", "descending", "dynamic", "from", "get", "global", "group", "into", "join", "let", "orderby", "partial", "remove", "select", "set", "value", "var", "where", "yield", "цел", "бцел", "крат", "бкрат", "байт", "ббайт", "бит", "т_мера", "реал", "вреал", "креал","ref", "min", "max", "length", "version", "export", "проц", "ук", "сим", "дим", "шим", "ткст", "шткст", "юткст", "template", "debug"};
        string[] methods = { "opCat()", "opApply()", "GetType()", "вТкст()" };
        string[] snippets = { "if(^)\n{\n;\n}", "if(^)\n{\n;\n}\nelse\n{\n;\n}", "for(^;;)\n{\n;\n}", "while(^)\n{\n;\n}", "do\n{\n^;\n}while();", "switch(^)\n{\ncase : break;\n}" };
        string[] declarationSnippets = { 
             "extern(D) class^\n{\n}","export extern(D) class^\n{\n}", "class ^\n{\n}", "public class ^\n{\n}", "private class ^\n{\n}", "internal class ^\n{\n}",
              "struct ^\n{\n;\n}", "public struct ^\n{\n;\n}", "private struct ^\n{\n;\n}", "internal struct ^\n{\n;\n}",
               "public void ^()\n{\n;\n}", "private void ^()\n{\n;\n}", "internal void ^()\n{\n;\n}", "protected void ^()\n{\n;\n}",
      ...
      private CompilerGUI compilman;
      //private FastColoredTextBox[]  openedtb;
      private  Language currentLang;
      private  bool langChanged= false;


      public string DINRUS;
      public static bool confOpened = false;


        public PowerfulEditor()
        {
            InitializeComponent();
            GetSetDinrus();
            //init menu images
        ...
        private void GetSetDinrus()
        {
            DINRUS = System.Environment.GetEnvironmentVariable("DINRUS");
            if (DINRUS == null)
            {
                System.Environment.SetEnvironmentVariable("DINRUS", System.Environment.CurrentDirectory + "\\bin", System.EnvironmentVariableTarget.Machine);
                DINRUS = System.Environment.GetEnvironmentVariable("DINRUS");
                MessageBox.Show("Переменная DINRUS не была установлена; автоматически ей назначено значение " + DINRUS);
                // this.Close();
            }
        }
        ...
        //Если изменён язык, нужно изменить и способ подсветки на вкладке
        //Для этого придётся её закрыть и открыть заново (?)
        /*
        ...
        */
        ...
        void changeLanguage()
        {
            ...
            switch (hLangTSCombo.Text)
            {
                case "C#": currentLang = Language.CSharp;
                    langChanged = true;
                    break;
                case "JScript": currentLang = Language.JS;
                    langChanged = true;
                    break;
                case "PHP": currentLang = Language.PHP;
                    langChanged = true;
                    break;
                case "HTML": currentLang = Language.HTML;
                    langChanged = true;
                    break;
                case "SQL": currentLang = Language.SQL;
                    langChanged = true;
                    break;
                case "VB": currentLang = Language.VB;
                    langChanged = true;
                    break;
                case "Другой": currentLang = Language.Custom;
                    langChanged = true;
                    break;
                case "Динрус": currentLang = Language.Dinrus;
                    langChanged = true;
                    break;
                default: currentLang = Language.Dinrus;
                    langChanged = true;
                    break;
            }
        }
        ...
        void tb_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            string text = r.GetFragment("[а-яА-Яa-zA-Z]}").Text;
        ...
        private void tsFiles_TabStripItemClosing(TabStripItemClosingEventArgs e)
        {
            ...
            {
                switch(MessageBox.Show("Сохранить ли " + e.Item.Title + " ?", "Сохранить", MessageBoxButtons.YesNoCancel, MessageBoxIcon.Information))
                {
        ...
        private bool Save(FATabStripItem tab)
        {
            ...
            {
                if (MessageBox.Show(ex.Message, "Ошибка", MessageBoxButtons.RetryCancel, MessageBoxIcon.Error) == DialogResult.Retry)
                    return Save(tab);
                else
                    return false;
            }
        ...
        private void tmUpdateInterface_Tick(object sender, EventArgs e)
        {
            try
            {
                if(CurrentTB != null && tsFiles.Items.Count>0)
                {
                    ...
                    saveToolStripButton.Enabled = saveTSMI.Enabled = tb.IsChanged;
                ...
                {
                    saveToolStripButton.Enabled = saveTSMI.Enabled = false;
            ...
            try
            {
                ...
                if (e.ColumnIndex == 1)
                    ...
                    switch (item.type)
                    {
                        ...
                        //    e.Value = global::DinrusIDE.Properties.Resources.class_libraries;
                            return;
                        case ExplorerItemType.Method:
                          //  e.Value = global::DinrusIDE.Properties.Resources.box;
                            return;
                        case ExplorerItemType.Event:
                           // e.Value = global::DinrusIDE.Properties.Resources.lightning;
                            return;
                        case ExplorerItemType.Property:
                           // e.Value = global::DinrusIDE.Properties.Resources.property;
                            return;
                    }
            }
            catch{;}
        ...
        private bool NavigateBackward()
        {
            ...
            if (iLine >= 0)
            {
                ...
                Console.WriteLine("Назад: " + lastNavigatedDateTime);
                tb.Focus();
                tb.Invalidate();
                return true;
            }
        ...
        private bool NavigateForward()
        {
            ...
            if (iLine >= 0)
            {
                ...
                tb.Navigate(iLine);
                lastNavigatedDateTime = tb[iLine].LastVisit;
                Console.WriteLine("Вперёд: " + lastNavigatedDateTime);
                tb.Focus();
                tb.Invalidate();
                return true;
            }
        ...
        class InsertEnterSnippet : AutocompleteItem
        {
            ...
            public override string ToolTipTitle
            {
                get
                {
                    return "Вставьте перенос строки после '}'";
                }
            }
        }
        ...
        {
            AboutBox1 aboutIt = new AboutBox1();
            aboutIt.Show();
        }

        private void справкаDV1ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            System.Diagnostics.Process.Start(DINRUS + "\\..\\help\\dhelp.chm");
        }

        private void справкаWIN32ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            System.Diagnostics.Process.Start(DINRUS + "\\..\\help\\win32.chm");
        }
        ...
        private void hLangTSCombo_SelectedIndexChanged(object sender, EventArgs e)
        {
            changeLanguage();
            langChanged = true;
        }

        private void hLangTSCombo_TextChanged(object sender, EventArgs e)
        {
            changeLanguage();
            langChanged = true;
        }
        ...
        private void динрусDMDToolStripMenuItem_Click(object sender, EventArgs e)
        {
            compilman = new CompilerGUI();
            compilman.Show();
        }
        ...
        private void gotoButton_Click_1(object sender, EventArgs e)
        {
        ...
        {
            var dlgWnd = new BestCS.FindForm((FastColoredTextBox)tsFiles.ActiveControl);
            dlgWnd.Show();
        }
    }
